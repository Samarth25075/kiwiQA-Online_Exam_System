        ...
        <AdminLayout>
            <div className="db-container" style={{ padding: '24px' }}>
                <style>{`
                    ...
                    .stat-pill.invited { background: var(--bg-neutral); color: var(--text-muted); border: 1px solid var(--border); }
                    .stat-pill.sat { background: var(--color-success-light); color: var(--color-success); border: 1px solid var(--color-success-border); }
                    .stat-pill.not-sat { background: var(--color-danger-light); color: var(--color-danger); border: 1px solid var(--color-danger-border); }
                    ...
                    .status-badge.sat { background: var(--color-success-light); color: var(--color-success); border: 1px solid var(--color-success-border); }
                    .status-badge.not-sat { background: var(--color-danger-light); color: var(--color-danger); border: 1px solid var(--color-danger-border); }
                ...
                {loading ? (
                    ...
                    <div className="empty-tracking" style={{ borderColor: 'rgba(220, 38, 38, 0.2)', background: 'rgba(220, 38, 38, 0.02)' }}>
                        ...
                        <h4 style={{ margin: '0 0 8px', color: 'var(--color-danger)' }}>Failed to load data</h4>
                        <p style={{ margin: '0 0 16px', color: 'var(--text-muted)' }}>{error}</p>
                        <button
                            onClick={fetchData}
                    ...
                    <div className="empty-tracking">
                        <div style={{ fontSize: '40px', marginBottom: '16px' }}>✉️</div>
                        <h4 style={{ margin: '0 0 8px', color: 'var(--text)' }}>No invitations sent yet</h4>
                        <p style={{ margin: 0, color: 'var(--text-muted)' }}>When you send exam links through the dashboard, you'll see tracking reports here.</p>
                    </div>
                ) : (
                            ...
                            <div key={exam.exam_id} className={`tracking-card ${expandedExam === exam.exam_id ? 'expanded' : ''}`}>
                                ...
                                >
                                    ...
                                    <div className="tracking-stats">
                                        ...
                                        <div style={{ marginLeft: '10px', color: 'var(--text-muted)' }}>
                                            ...
                                            <table className="details-table">
                                                ...
                                                <tbody>
                                                    {exam.details.map((detail, idx) => (
                                                        <tr key={idx}>
                                                            <td style={{ fontWeight: 600, color: 'var(--text)' }}>{detail.email}</td>
                                                            <td style={{ color: 'var(--text-muted)' }}>{new Date(detail.sent_at).toLocaleDateString()} at {new Date(detail.sent_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</td>
                                                            <td style={{ color: 'var(--text-muted)' }}>{detail.admin_name || 'Admin'}</td>
                                                            <td>
