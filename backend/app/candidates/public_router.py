...
async def set_candidate_status(token: str, status: str, db = Depends(get_db)):
    """Update candidate status electronically (Live/Completed)."""
    success = await update_candidate_status(db, token, status)
    if not success:
        raise HTTPException(status_code=404, detail="Candidate not found")
    return {"message": f"Status updated to {status}"}

@router.post("/test/{token}/submit")
async def submit_test(token: str, result: CandidateResult, background_tasks: BackgroundTasks, db = Depends(get_db)):
    ...
    success = await update_candidate_result(
...
async def resend_test_results(token: str, background_tasks: BackgroundTasks, db = Depends(get_db)):
...
async def get_test(token: str, device_id: Optional[str] = None, db = Depends(get_db)):
    """Public endpoint for candidates to fetch their assigned exam."""
    # Use optimized summary fetch
    candidate = await get_candidate_by_token_summary(db, token)
    ...
    if device_id:
        ...
        if await is_device_used_for_exam(db, device_id, candidate["email"], candidate["assigned_exam_id"]):
        ...
        if not saved_device:
            await update_candidate_details(db, candidate["id"], {"device_id": device_id})
...
async def request_enroll_otp(exam_id: str, req: CandidateEnrollOTPRequest, background_tasks: BackgroundTasks, db = Depends(get_db), device_id: Optional[str] = None):
    """Generate and send an OTP for candidate self-enrollment."""
    await get_exam_and_check_expiry(db, exam_id)
    
    # Optimized direct DB checks
    already_registered = await db.candidates.find_one({
        "email": req.email,
    ...
    if device_id:
        device_conflict = await db.candidates.find_one({
            "device_id": device_id,
            "email": {"$ne": req.email},
            "assigned_exam_id": exam_id
        }) is not None
...
async def verify_enroll_otp(exam_id: str, req: CandidateEnrollOTPVerify, db = Depends(get_db)):
    ...
    existing = await db.candidates.find_one({"email": req.email, "assigned_exam_id": exam_id})
    if existing:
    ...
    candidate = await create_candidate(
        ...
        admin_name="Self Enrolled",
        device_id=str(req.device_id or "")
    )
    
    updated = await assign_exam_to_candidate(db, str(candidate["id"]), exam_id)
    if not updated:
...
async def get_public_test_results(token: str, db = Depends(get_db)):
    ...
    candidate_obj = await db.candidates.find_one({"token": token})
